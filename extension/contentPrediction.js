...
class AdVolumeDetector {
  ...
  async loadModel() {
    ...
    try {
      ...
      try {
        ...
        if (lrResponse.ok) {
          const jsModel = await lrResponse.json();
          this.model = new SimpleJSModel(jsModel);
          console.log('Ad Volume Reducer: logistic regression model loaded');
          return;
        }
      } catch (error) {
  ...
  async setupAudioAnalysis() {
    ...
    try {
      ...
      // Reuse existing AudioContext and sourceNode if possible
      if (
        this.audioContext &&
        this.audioContext.state !== 'closed' &&
        this.sourceNode &&
        this.sourceNode.mediaElement === video
      ) {
        // Already set up for this video, just ensure analyser/gain are connected
        if (!this.analyser) {
        ...
        // If not valid, create new AudioContext and source node
        if (!this.audioContext || this.audioContext.state === 'closed') {
